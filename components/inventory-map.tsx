              ...
              <div className="space-y-4">
                <section>
                  <div className="mb-2 flex items-center justify-between gap-2">
                    ...
                    <Link href={`/inventory/${activeResource.id}`} className="shrink-0 rounded-lg border border-border px-2.5 py-1.5 text-[11px] font-semibold text-muted hover:bg-surface-hover">{t("map.resource.open")}</Link>
                  </div>

                  {isEditing ? (
                    <div className="rounded-xl border border-border bg-surface-subtle p-2.5">
                      <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">{t("map.draw.title")}</p>
                      ...
                      {drawMode !== "idle" ? (
                        <div className="mt-2 rounded-lg bg-surface p-2 text-[11px] leading-4 text-muted ring-1 ring-border">
                          {drawMode === "point"
                ...
                {activeFeatures.length ? (
                  <section>
                    <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">{t("map.features.title")}</p>
                    <div className="space-y-1">
                      {activeFeatures.map((feature, index) => (
                        <button key={feature.id} type="button" onClick={() => setActiveFeatureId(feature.id)} className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs ${activeFeatureId === feature.id ? "bg-brand-soft text-brand ring-1 ring-inset ring-brand-border" : "bg-surface-subtle text-muted hover:bg-surface-hover"}`}>
                          ...
                          <span className="text-[11px] text-muted">{integer.format(index + 1)}</span>
                        </button>
                      ))}
                    </div>
                  </section>
                ...
                {activeFeature && isEditing ? (
                  <section className="rounded-xl border border-warning-border bg-warning-soft p-3">
                    ...
                    <label className="block text-[11px] font-semibold text-muted">{t("map.features.layer")}<input value={activeFeature.layer} onChange={(event) => updateActiveFeature({ layer: event.target.value })} className={`${inputClass} mt-1`} /></label>
                    <label className="mt-2 block text-[11px] font-semibold text-muted">{t("map.features.description")}<textarea value={activeFeature.description} onChange={(event) => updateActiveFeature({ description: event.target.value })} rows={2} className={`${inputClass} mt-1 h-auto py-2`} /></label>
                    <p className="mt-2 text-[11px] leading-4 text-muted">{t("map.features.editHint")}</p>
                  </section>
                ) : null}

                {isEditing && selectedIds.length ? (
                  <section className="border-t border-border pt-4">
                    <div className="mb-2 flex items-center gap-2"><MousePointer2 size={13} className="text-brand" /><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">{t("map.quickEdit.title", { count: selectedIds.length, value: integer.format(selectedIds.length) })}</p></div>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <label className="col-span-2 flex items-center gap-2 rounded-lg bg-surface-subtle px-2.5 py-2 text-[11px] font-semibold text-muted"><input type="checkbox" checked={applyLocation} onChange={(event) => setApplyLocation(event.target.checked)} className="accent-brand-solid" /> {t("map.quickEdit.changeLocation")}</label>
        ...
        <section className="relative order-1 min-h-[560px] overflow-hidden bg-surface-muted lg:order-2">
          ...
          {structures.length ? (
            <div className="absolute right-14 top-3 z-10 w-[min(300px,calc(100%-72px))] rounded-xl border border-border bg-surface/95 p-2 shadow-lg backdrop-blur">
              <div className="flex items-center gap-2 px-1 pb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-muted">
                ...
                <div className="mt-2 flex items-center gap-1.5 overflow-x-auto">
                  ...
                  {structureDetail.floors.map((floor) => (
                    <button
                      ...
                      className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold ${activeFloorIdentifier === floorIdentifier(floor.identifier, floor.index) ? "bg-brand-solid text-on-brand" : "bg-surface-muted text-muted hover:bg-surface-hover"}`}
                    >
              ...
              {activeStructureId ? (
                <Link
                  ...
                  className="mt-2 flex h-8 items-center justify-center rounded-lg bg-strong text-[11px] font-semibold text-on-strong hover:opacity-90"
                >
          ...
          <div className="absolute left-3 top-3 z-10 flex rounded-xl border border-border bg-surface/95 p-1 shadow-lg backdrop-blur">
            <button type="button" onClick={() => setBasemap("streets")} className={`inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[12px] font-semibold ${basemap === "streets" ? "bg-strong text-on-strong" : "text-muted hover:bg-surface-hover"}`}><MapIcon size={13} /> {t("map.basemap.map")}</button>
            <button type="button" onClick={() => setBasemap("satellite")} className={`inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[12px] font-semibold ${basemap === "satellite" ? "bg-strong text-on-strong" : "text-muted hover:bg-surface-hover"}`}><Satellite size={13} /> {t("map.basemap.satellite")}</button>
          </div>
          ...
          {isEditing ? (
            <div className="pointer-events-none absolute bottom-3 right-3 z-10 hidden rounded-lg bg-surface/90 px-2.5 py-1.5 text-[11px] text-muted shadow sm:block">
              {t("map.shortcuts")}
            </div>
          ) : null}
        </section>
