...
function OptionEditor({
  ...
  return (
    <div className="space-y-4 p-4 sm:p-5">
      <div className="rounded-xl border border-brand-border bg-brand-soft/35 px-4 py-3 text-[11px] leading-5 text-muted-strong">
          ...
          <div className="mt-4 space-y-2">
            {group.values.map((value, valueIndex) => (
              <div key={value.localId} className="grid gap-2 rounded-lg bg-surface-subtle p-2 sm:grid-cols-[auto_1fr_0.65fr_1.25fr_auto] sm:items-center">
                <label className="flex items-center gap-2 text-[11px] font-medium text-muted-strong">
                ...
                {group.bomSlotKey ? (
                  ...
                  <span className="px-2 text-[11px] text-muted">{t("options.editor.catalogOnly")}</span>
                )}
                <button
                  type="button"
