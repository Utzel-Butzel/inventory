...
function CoverReferencePicker({
  ...
  return (
    <fieldset className="mt-3" disabled={disabled}>
      <legend className="text-[12px] font-semibold text-muted-strong">
        {t("ai.referenceImage")}
      </legend>
          ...
          return (
            ...
            >
              ...
              <span className="flex min-w-0 items-center gap-1 px-1.5 py-1">
                ...
                <span className="truncate text-[10px] font-medium text-muted-strong">
                  {option.label}
                </span>
              </span>
              {option.isAi ? (
                <span className="absolute right-1 top-1 rounded bg-brand-solid px-1 py-0.5 text-[9px] font-bold text-on-brand">
                  {t("media.ai")}
                </span>
              ) : null}
            </label>
      ...
      {variantContext ? (
        <div className="mb-5 flex items-start gap-3 rounded-2xl border border-brand-border bg-brand-soft/45 px-4 py-3.5">
          ...
          {variantContext.overriddenFields.length ? (
            <span className="shrink-0 rounded-full border border-brand-border bg-surface px-2.5 py-1 text-[11px] font-semibold text-brand">
              ...
              <div className="sm:col-span-2">
                <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex h-11 shrink-0 items-center justify-center gap-1.5 rounded-lg border border-border bg-surface px-2.5 text-[12px] font-semibold text-muted-strong transition hover:bg-surface-muted disabled:opacity-50 sm:h-8"
                  >
          ...
          <section className="rounded-2xl border border-border bg-surface p-5 shadow-[var(--shadow-sm)] sm:p-6">
            <div className="mb-5 flex items-center justify-between gap-4 border-b border-border pb-4">
              ...
              {!customFieldsLoading && !customFieldsError ? (
                <span className="hidden rounded-full bg-brand-soft px-2.5 py-1 text-[11px] font-semibold text-brand sm:inline-flex">
                  ...
                  return (
                    <div key={item.id} className="group relative overflow-hidden rounded-2xl border border-border bg-surface-subtle">
                      ...
                      <div className="absolute inset-x-2 top-2 flex items-center justify-between">
                        <span className={`rounded-full px-2 py-1 text-[11px] font-bold ${index === 0 ? "bg-success text-on-strong" : "bg-surface/90 text-muted"}`}>
                          {index === 0 ? t("media.cover") : index + 1}
                        </span>
                        {item.source === "ai" ? <span className="rounded-full bg-brand-solid px-2 py-1 text-[11px] font-bold text-on-brand">{t("media.ai")}</span> : null}
                      </div>
        ...
        <aside className="space-y-5 xl:sticky xl:top-5">
          ...
          <section className="rounded-2xl border border-border bg-surface p-5">
            ...
            {isNew ? (
              <div className="space-y-3">
                ...
                <p className="text-[12px] leading-4 text-muted">{t("ai.safety")}</p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                {!hasImage ? <p className="text-[12px] text-warning">{t("ai.imageRequired")}</p> : null}
                <p className="text-[12px] leading-4 text-muted">{t("ai.modalHint")}</p>
              </div>
            )}
          </section>
          ) : null}
          ...
          <details className="group rounded-2xl border border-border bg-surface p-5">
            ...
            {!isNew && resourceId ? (
              ...
              <p className="mb-3 text-[12px] leading-4 text-muted">{t("position.saveFirst")}</p>
            )}
            {resource?.mapFeatures.length ? <p className="mb-3 text-[12px] leading-4 text-brand">{t("position.geometryNotice")}</p> : null}
            ...
            {mapHref ? <a href={mapHref} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-success hover:text-success">{t("position.openMap")} <ChevronRight size={13} /></a> : <p className="mt-3 text-[12px] leading-4 text-muted">{t("position.gpsHelp")}</p>}
            </div>
          </details>
          ) : null}
          ...
          {!isNew && resource ? (
            <section className="rounded-2xl border border-border bg-surface p-5">
              ...
              <button type="button" onClick={() => void navigator.clipboard.writeText(resource.id).then(() => setNotice(t("notices.idCopied")))} className="flex w-full items-center justify-between rounded-xl bg-surface-subtle px-3 py-2.5 text-left"><span className="min-w-0"><span className="block text-[11px] text-muted">{t("record.id")}</span><span className="block truncate font-mono text-xs text-muted">{resource.id}</span></span><Copy size={14} className="ml-3 shrink-0 text-muted" /></button>
            </section>
          ) : null}
        </aside>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {([
            ...
            <div key={titleKey} className="rounded-2xl border border-border bg-surface-subtle p-4">
              ...
              <p className="mt-1 text-[12px] leading-5 text-muted">{t(descriptionKey)}</p>
            </div>
          ))}
        </div>
        <p className="rounded-xl border border-success-border bg-success-soft px-3 py-2.5 text-[12px] leading-5 text-success">
      ...
      >
        ...
        {imageAcquisitionMode === "search" ? (
          <>
            ...
            <p className="rounded-xl border border-warning-border bg-warning-soft px-3 py-2.5 text-[12px] leading-5 text-warning">
