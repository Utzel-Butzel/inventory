...
function Runway({ item, compact = false }: { item: StockItem; compact?: boolean }) {
  ...
  return (
    <div className={cn(compact && "flex items-center justify-between gap-3")}>
      <p className={cn("flex items-center gap-1.5 text-[13px] font-semibold", color)}>
        <span className={cn("size-1.5 rounded-full", dot)} />
        {label}
      </p>
      {predictedDate ? (
        <p className={cn("text-[11px] text-muted", !compact && "mt-0.5")}>
        ...
        <div className="space-y-5">
          ...
          <Card className="overflow-hidden">
              ...
              <>
                <div className="hidden overflow-x-auto lg:block">
                  <table className="w-full min-w-[960px] border-collapse text-left">
                    <thead>
                      <tr className="border-b border-border bg-surface-subtle text-[11px] font-semibold uppercase tracking-[0.09em] text-muted">
                        ...
                        return (
                          ...
                          >
                            <td className="px-5 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                  <Link href={`/inventory/${item.resourceId}/stock`} className="block max-w-[260px] truncate text-[14px] font-semibold text-foreground transition hover:text-brand">
                                    {item.name}
                                  </Link>
                                  <p className="mt-0.5 text-[11px] text-muted">
                            ...
                            <td className="px-4 py-4">
                              <p className={cn("text-[14px] font-semibold tabular-nums", state === "out" ? "text-danger" : "text-foreground")}>
                            ...
                            <td className="px-4 py-4">
                              {item.onOrder > 0 ? (
                                <div>
                                  <p className="text-[13px] font-semibold tabular-nums text-info">
                            ...
                            <td className="px-4 py-4 text-[13px] font-medium tabular-nums text-muted">
                            ...
                            <td className="px-4 py-4 text-[13px] font-medium text-muted">
                              {formatUsage(
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-surface/80 p-3 ring-1 ring-inset ring-border">
                              <div>
                                ...
                                <p className={cn("mt-1 text-[14px] font-semibold tabular-nums", state === "out" ? "text-danger" : "text-foreground")}>
                                  {formatQuantity(item.quantity, item.unitName, compactNumber)}
                                </p>
                              </div>
                              <div>
                                ...
                                <p className="mt-1 text-[14px] font-semibold tabular-nums text-muted-strong">
                                  {item.minimumStock === null
                                    ? t("overview.table.notSet")
                                    : formatQuantity(
                                        item.minimumStock,
                              ...
                              <div>
                                ...
                                <p className="mt-1 text-[14px] font-semibold tabular-nums text-info">
                                  {item.onOrder > 0
                                    ? `+${formatQuantity(
                                        item.onOrder,
                                        item.unitName,
            ...
            {filteredItems.length ? (
              <div className="flex flex-col gap-2 border-t border-border bg-surface-subtle px-4 py-3 text-[12px] text-muted sm:flex-row sm:items-center sm:justify-between sm:px-5">
                <span>
