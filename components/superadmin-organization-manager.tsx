...
export function SuperadminOrganizationManager() {
  ...
  const [editingAllowNegativeStock, setEditingAllowNegativeStock] =
    useState(false);
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          errorMessage(payload, t("superadminOrganizations.errors.load")),
        );
      }
      ...
      setLoadError(
        error instanceof Error
          ? error.message
          : t("superadminOrganizations.errors.load"),
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  async function createOrganization(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          errorMessage(payload, t("superadminOrganizations.errors.create")),
        );
      }
      setNewName("");
      setNewSlug("");
      setCreateOpen(false);
      setNotice(t("superadminOrganizations.notices.created", { name }));
      await load(true);
    } catch (error) {
      setActionError(
        error instanceof Error
          ? error.message
          : t("superadminOrganizations.errors.create"),
      );
    } finally {
      setCreating(false);
    }
  ...
  async function updateOrganization(
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/organizations/${encodeURIComponent(organization.id)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!response.ok) {
        throw new Error(
          errorMessage(payload, t("superadminOrganizations.errors.update")),
        );
      }
      closeEditor();
      setNotice(t("superadminOrganizations.notices.updated", { name }));
      await load(true);
    } catch (error) {
      setActionError(
        error instanceof Error
          ? error.message
          : t("superadminOrganizations.errors.update"),
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="flex flex-col gap-4 border-b border-border px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="font-semibold text-foreground">
                  {t("superadminOrganizations.list.title")}
                </h2>
                {!loading ? (
                  <Badge>
                    {t("superadminOrganizations.list.count", {
              ...
              <p className="mt-1 text-sm text-muted">
                {t("superadminOrganizations.list.description")}
              </p>
            </div>
          </div>
          <Button
          ...
          >
            <RefreshCw
              ...
              aria-hidden="true"
            />
            {t("superadminOrganizations.actions.refresh")}
          </Button>
        </div>
        ...
        <div className="p-3 sm:p-4">
          {loading ? (
            ...
            <div className="grid min-h-52 place-items-center p-6 text-center">
              <div>
                ...
                <p className="mt-3 text-sm font-semibold text-foreground">
                  {t("superadminOrganizations.errors.loadTitle")}
                </p>
                ...
                >
                  {t("superadminOrganizations.actions.retry")}
                </button>
              </div>
            </div>
            ...
            <EmptyState
              ...
              title={t("superadminOrganizations.list.emptyTitle")}
              description={t("superadminOrganizations.list.emptyDescription")}
            />
          ) : (
                      ...
                      <form onSubmit={(event) => void updateOrganization(event, organization)}>
                        ...
                        <label className="mt-3 block">
                          ...
                          <div className="mt-2 flex h-11 items-center rounded-xl border border-border bg-surface px-3.5 shadow-sm focus-within:border-focus focus-within:ring-4 focus-within:ring-focus/10">
                            ...
                            <input
                              value={editingSlug}
                              onChange={(event) => setEditingSlug(event.target.value.toLowerCase())}
                              required
                        ...
                        <label className="mt-3 flex items-start gap-3 rounded-xl border border-border bg-surface-subtle p-3">
                          <input
                            ...
                            onChange={(event) => setEditingAllowNegativeStock(event.target.checked)}
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {organization.isReadOnly ? (
                              <Badge tone="warning">
                                {t("superadminOrganizations.list.readOnly")}
                              </Badge>
                            ) : null}
      ...
      <Card className="h-fit overflow-hidden">
        <div className="border-b border-border px-5 py-5 sm:px-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="font-semibold text-foreground">
                {t("superadminOrganizations.create.title")}
              </h2>
              <p className="mt-1 text-sm leading-5 text-muted">
                {t("superadminOrganizations.create.description")}
              </p>
            </div>
            {!createOpen ? (
        ...
        {createOpen ? (
          ...
          <div className="px-5 py-5 text-xs leading-5 text-muted sm:px-6">
            {t("superadminOrganizations.create.hint")}
          </div>
        )}
      </Card>
    </div>
