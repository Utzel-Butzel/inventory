    ...
    <section className="overflow-hidden rounded-xl border border-brand-border bg-surface">
      <div className="bg-gradient-to-br from-brand-soft via-surface to-surface px-5 pb-4 pt-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className="rounded-full bg-success-soft px-2.5 py-1 text-[11px] font-semibold text-success ring-1 ring-inset ring-success-border">
            ...
            <div className="mt-2 flex flex-wrap gap-1.5" aria-label={resourceT("details.slugs")}>
              {resource.slugs.map((slug, index) => (
                <Link
                  key={slug}
                  href={`/inventory/${slug}`}
                  className={`rounded-md px-2 py-1 font-mono text-[12px] transition ${
                    index === 0
              ...
              <div className="grid border-b border-border lg:grid-cols-2">
                {articleImage ? (
                  <div className="relative aspect-square self-start overflow-hidden bg-[radial-gradient(circle_at_50%_35%,var(--color-surface),var(--color-surface-muted))]">
                    ...
                    />
                    <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-surface/90 px-2.5 py-1 text-[11px] font-semibold text-muted shadow-sm">
              ...
              >
                ...
                {isRoom && roomScan ? (
                  <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 bg-gradient-to-t from-black/75 via-black/25 to-transparent px-5 pb-5 pt-16 text-white">
                    ...
                    <span className="shrink-0 rounded-full bg-black/35 px-2.5 py-1 text-[11px] font-semibold text-white/85 ring-1 ring-inset ring-white/20 backdrop-blur-sm">
                      {t("details.roomScan.revision", {
