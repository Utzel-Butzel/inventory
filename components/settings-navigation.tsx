    ...
    <>
      ...
      <div className="border-b border-border bg-surface px-4 py-3 xl:hidden">
        <details className="group relative">
          <summary className="flex h-10 cursor-pointer list-none items-center justify-between rounded-xl border border-border bg-surface px-3 text-[14px] font-semibold text-foreground [&::-webkit-details-marker]:hidden">
          ...
          >
            {visibleGroups.map((group) => (
              <div key={group.labelKey}>
                <p className="px-2 py-1 text-[12px] font-semibold text-muted">
                  {t(group.labelKey)}
                </p>
                {group.items.map((item) => {
                  const active = isActive(pathname, item);
                  ...
                  return (
                    <Link
                      ...
                      aria-current={active ? "page" : undefined}
                      className={cn(
                        "flex min-h-10 items-center gap-3 rounded-lg px-2.5 text-[14px] font-medium",
                        active
                          ? "bg-brand-soft text-brand"
