    ...
    <Card className="overflow-hidden">
      ...
      <div className="grid gap-3 p-5 sm:grid-cols-3 sm:p-6">
        <div className="rounded-xl border border-border bg-surface-subtle p-3.5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted">{t("locations.metrics.total")}</p>
          <p className="mt-1.5 text-xl font-semibold tracking-tight text-foreground">
            {numberFormat.format(totalQuantity)}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-surface-subtle p-3.5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted">{t("locations.metrics.assigned")}</p>
          <p className="mt-1.5 text-xl font-semibold tracking-tight text-foreground">
            {numberFormat.format(data.breakdown.assignedQuantity)}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-surface-subtle p-3.5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted">{t("locations.unassigned")}</p>
          <p className="mt-1.5 text-xl font-semibold tracking-tight text-foreground">
      ...
      {canEdit ? (
        <form onSubmit={submitTransfer} className="p-5 sm:p-6">
          <div className="mb-4 flex items-start justify-between gap-3">
            ...
            <span className="rounded-full bg-surface-muted px-2.5 py-1 text-[11px] font-semibold capitalize text-muted">
              {t(`locations.tracking.${data.breakdown.trackingMode}`)}
            </span>
          </div>

          {data.breakdown.trackingMode === "serialized" ? (
            <div className="flex items-start gap-2 rounded-xl border border-info-border bg-info-soft px-3.5 py-3 text-[12px] leading-4 text-info">
              <AlertTriangle className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
              {t("locations.move.serializedHelp")}
            </div>
          ) : sourceOptions.length === 0 ? (
            <div className="flex items-start gap-2 rounded-xl border border-border bg-surface-subtle px-3.5 py-3 text-[12px] leading-4 text-muted">
              <PackageOpen className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
              {t("locations.move.noStock")}
            </div>
          ) : !hasTransferDestination ? (
            <div className="flex items-start gap-2 rounded-xl border border-warning-border bg-warning-soft px-3.5 py-3 text-[12px] leading-4 text-warning">
        ...
        <div className="px-5 py-4 text-[12px] leading-5 text-muted sm:px-6">
