                      ...
                      {searchResults.map((resource) => (
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block truncate text-[14px] font-semibold text-foreground">{resource.name}</span>
                        ...
                        <div className="min-w-0 pt-0.5">
                          <div className="flex min-w-0 items-center gap-2">
                            <Link
                              ...
                              className="min-w-0 truncate text-[14px] font-semibold text-foreground hover:text-brand"
                            >
                              {component.name}
                            </Link>
                            {variantOrigin ? (
                              <Badge
                                ...
                                className="min-h-5 shrink-0 px-2 py-0 text-[11px]"
                              >
                                {t(
              ...
              <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[320px_minmax(0,1fr)]">
                <div className="space-y-4">
                  ...
                  {bom.resource.trackingMode === "serialized" ? (
                    <label className={labelClass}>
                      ...
                      />
                      <span className={cn("mt-1.5 block text-[11px]", outputCodesValid ? "text-muted" : "text-danger")}>
                ...
                <div className="min-w-0">
                  <div className="overflow-hidden rounded-xl border border-border">
                    <div className="hidden grid-cols-[minmax(180px,1fr)_90px_90px_90px] gap-3 border-b border-border bg-surface-subtle px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.09em] text-muted sm:grid">
                    ...
                    <div className="divide-y divide-border">
                      {preview.map((component) => (
                        <div key={component.slotKey} className={cn("grid gap-3 px-4 py-3 sm:grid-cols-[minmax(180px,1fr)_90px_90px_90px] sm:items-center", component.blockingShortage && "bg-danger-soft")}>
                          ...
                          <div className="flex items-center justify-between sm:block"><span className="text-[10px] uppercase text-muted sm:hidden">{t("assembly:labels.afterBuild")}</span><span className={cn("text-[13px] font-semibold tabular-nums", component.remaining < 0 ? "text-danger" : component.remaining === 0 ? "text-warning" : "text-success")}>{component.remaining}</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
                          ...
                          {component.availableUnits.map((unit) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="min-w-0"><span className="block truncate font-mono text-[11px] font-semibold">{unit.code}</span><span className="mt-0.5 block truncate text-[10px] opacity-70">{unit.location || t("assembly:labels.noLocation")}</span></span>
                              </button>
                            );
                          })}
                        </div>
        ...
        <Card className="overflow-hidden">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {build.note ? <p className="mt-2 text-[12px] leading-5 text-muted">{build.note}</p> : null}
          ...
          {builds.length > 12 ? (
            <div className="border-t border-border bg-surface-subtle px-5 py-3 text-right">
              <Link href={`/inventory/${resourceId}/stock`} className="inline-flex items-center gap-1 text-[12px] font-semibold text-brand">
