...
export async function POST(request: Request) {
  ...
  const existing = await findRoomScan(identifiers.data.id);
  if (existing) {
    if (existing.roomResourceId !== identifiers.data.roomResourceId) {
      return Response.json(
        { error: "That scan identifier belongs to another room." },
        { status: 409 },
      );
    }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unable to save room scan.";
    const status = message.includes("another room")
      ? 409
